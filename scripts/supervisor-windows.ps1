...
function Start-Fallback {
    ...
    # Remove CLAUDECODE from process env so the daemon won't inherit it
    # (prevents "cannot launch inside another Claude Code session" error)
    Remove-Item Env:\CLAUDECODE -ErrorAction SilentlyContinue

    $errLog = Join-Path (Join-Path $CtiHome 'logs') 'bridge-err.log'
...
switch ($Command) {
    ...
    'stop' {
        ...
        if ($svc -and $svc.Status -eq 'Running') {
            ...
            $bPid = Read-Pid
            if (-not $bPid) { Write-Host "No bridge running"; exit 0 }
            if (Test-PidAlive $bPid) {
    ...
    'status' {
        ...
        if ($bPid -and (Test-PidAlive $bPid)) {
            Write-Host "Bridge process is running (PID: $bPid)"
